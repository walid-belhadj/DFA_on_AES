...
#inverse shit row decryption path
...
# normal shfit row encryption path
def forward_shift_row(in_list):
    ...
    output_list.append(in_list[11])
    return output_list
#file log to record all serach operation
fo = open('log.txt','w')

pot_msg1 = []

ciphertext = inverse_shift_row(ciphertextNormal)
faultyone = inverse_shift_row(faultytext0)
faultytwo = inverse_shift_row(faultytext1)
faultythree = inverse_shift_row(faultytext2)
for i in range(16):
    fo.write('================================================\n')
    for j in range(256):
        checker = 0
...
key10 = []
for j in range(16):
    key10.append(pot_msg3[j] ^ ciphertext[j])

print('===============================================')
print('Round 10 key is : ')
print(', '.join([hex(i) for i in key10]))

# inverse key schedule open file to record all shedule rounds keys  ( 10 rounds )
f_key = open('keys.txt','w')
f_key.write(', '.join([hex(i) for i in key10]))
f_key.write('\n')
#comparing values with rcon table
...
#matrix 4x4 of key saving into 16 bytes
...
print('===============================================')
print(' Inverse key schedule starts from here  ')
